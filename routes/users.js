...
const paystack = require('paystack')('sk_live_69690aa61315d92b683ad7afe157db049c3e54c3');
    ...
    User.checkUsername(req.body.username, (err, checkUser) => {
        ...
        User.checkMobileNo(req.body.mobile_no, (err, checkMobile) => {
            ...
            if (checkUser < 1 && checkMobile < 1) {
                ...
                res.json({ success: false, msg: 'Oops! looks like that username / mobile number is taken' });
            }
        });
    });
});
        ...
        User.comparePassword(req.body.old_password, user.password, (err, isMatch) => {
            ...
            if (isMatch) {
                User.updateUserProfile(updatedData, (response) => {
                    if (!response) {
                        res.json({ success: false, msg: 'Ooops! we seem to have issues updating your profile' });
                    } else {
                        res.json({ success: true, msg: 'Success! Your profile has been updated. Remember to use your new password on your next log in.' });
                    }
                });
            } else {
                return res.json({ success: false, msg: 'Wrong password' });
        ...
        if (error) {
            ...
            User.updateUserBalance(user_id, new_balance, (err, response) => {
                if (err) {
                    ...
                    res.json({ success: true, msg: 'Successfull! Your account has been credited' });
                }
            })
        }
    });
...
router.post('/payWinner', (req, res) => {
    ...
    User.payWinner(user_id, new_balance, (err) => {
        if (err) {
            res.json({ success: false, msg: 'Amount Won could not reflect, Please try later' });
        } else {
            res.json({ success: true, msg: 'Successfull! Your account has been credited with your winnings' });
        }
    })
});
