...
router.post('/newRecipient', (req, res) => {
    axios({
      ...
      .then((response) => {
        if (response.status) {
            let newRecipient = new tranferRecipient({
                user_id: req.body.user_id,
        ...
        //    console.log(newRecipient.user_id);
            transferRecipient.findRecipientByUserId(newRecipient.user_id , (error, count) => {
                if(error) throw error;
                    if (count > 0) {
                        tranferRecipient.updateRecipient(newRecipient, (response) => {
                            if (response) {
                                res.json({ success: true, 
                                    msg: "Your withdrawal account has been updated and verified!"
                            });  

                            } else {
                                res.json({ sucess: false, msg: 'Looks like we\'ve got a problem updating yuor account.' });
                        ...
                        transferRecipient.addNewRecipient(newRecipient, (err) => {
                            if (err) {
                                res.json({ sucess: false, msg: 'Sorry, we have issues verifying your account details.' });
                                console.log(err);
                            } else {
                                res.json({ success: true, 
