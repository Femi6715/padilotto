...
const endOfTheMonth = moment().endOf('month').format('YYYY-MM-DD');
const startOfTheMonth = moment().startOf('month').format('YYYY-MM-DD');
const endOfTheMonthReversed = moment().endOf('month').format('D-M-YYYY');
            ...
            db.collection('games').countDocuments({
                ...
                .then((count) => {
                    ...
                    db.collection('games').find({
                        ...
                        .toArray().then((docs) => {
                            ...
                            db.collection('games').findOneAndUpdate({
                                _id: docs[item]._id
                                    ...
                                    db.collection('users').findOne({ _id: ObjectID(docs[item].user_id) }, function (err, winner_info) {
                                        if (err) throw err;
                                        const winnerBalance = winner_info.main_balance;
                                        const winnerId = winner_info._id;
                                        let amount_won = parseInt(docs[item].potential_winning, 10);
                                        ...
                                        db.collection('users').findOneAndUpdate({
                                            _id: winnerId
                                                ...
                                                console.log(`TICKET STATUS UPDATE FAILED: ${status_err}`)
                                            });
                        ...
                        }).catch((err) => console.log('ERROR: could not fetch at this time' + err));
...
let shuffleMachine = (our_stake_amt) => {
    MongoClient.connect(config.database, { useNewUrlParser: true }, (err, client) => {
        if (err) {
            ...
            db.collection('games').countDocuments({ draw_date: shuffleAlert, ticket_status: ticket_status, stake_amt: stake_amt })
                .then((count) => {
                    this.totalCount = (count - 1);
                    db.collection('games').find({ draw_date: shuffleAlert, ticket_status: 'pending', stake_amt: stake_amt }, { skip: (this.totalCount) })
                        .toArray().then((docs) => {
                            const obj = Object.keys(docs);
                            item = obj[Math.floor(Math.random() * obj.length)];
